...
def ppo_loss(y_true, y_pred):
    ...
    actions = nn.functional.one_hot(actions)
    LOSS_CLIPPING = 0.2
    #C1 = 1
    C2 = 5e-3
    epsilon = 1e-10
    ...
    prob = y_pred * actions
    old_prob = torch.vstack(prediction_picks) * actions
...
class A3CAgent:
    ...
    def act(self, state):
        ...
        return action.item(), prediction[0].clone().detach()
    ...
    def PlotModel(self, score, episode):
        ...
        if str(episode)[-2:] == "00":# much faster than episode % 100
            ...
            try:
                pylab.savefig(self.save_path+"PPO.png")
            except OSError:
                pass
    ...
    def train_threading(self, agent, env, thread):
            while self.episode < self.EPISODES:
                ...
                while not done:
                    action, prediction = agent.act(torch.from_numpy(state))
    ...
    def run(self):
        ...
        while self.episode < self.EPISODES:
            ...
            states, actions, rewards, predictions = [], [], [], []
            while not done:
                action, prediction = self.act(torch.from_numpy(state))
                next_state, reward, done, _ = self.step(action, env, state)
    ...
    def test(self):
        ...
        for e in range(100):
            ...
            while not done:
                env.render()
                action, _ = self.act(torch.from_numpy(state))
