...
class Critic_Model:
    ...
    def save_weights(self):
        ...
        np.save("critic_weights.dmp", weights)

    def load_weights(self):
        self.Critic.set_weights(np.load("critic_weights.dmp.npy", allow_pickle=True))
...
class PPOAgent:
    ...
    def run_dump(self, save = False): # train only when episode is finished
        ...
        while True:
            # Instantiate or reset games memory
            states = [] if save else np.load("states.dmp.npy", allow_pickle = True)
            next_states = [] if save else np.load("next_states.dmp.npy", allow_pickle = True)
            rewards = [] if save else np.load("rewards.dmp.npy", allow_pickle = True)
            dones = [] if save else np.load("dones.dmp.npy", allow_pickle = True)
            actions, predictions = [], []
            idx = 0
            while not done:
                # Actor picks an action
            ...
            if save:
                ...
                np.save("rewards.dmp", rewards)
                np.save("dones.dmp", dones)
            break
            #if self.episode >= self.EPISODES:
                #break
        self.env.close()
